...
/** Id della schermata attualmente visibile, null prima della prima chiamata a showScreen(). */
...
async function showScreen(targetId, options = {}) {
  ...
  // assoluto (isInitialReveal) non c'è nulla da nascondere: screen-landing
  // è già visibile di default nell'HTML statico, senza bisogno di animarla.
...
/**
 * Risolve la Promise alla prima delle due condizioni: un click ovunque
 * dentro `screenElement`, oppure lo scadere di `timeoutMs`. Usata dalla
 * schermata alert (vedi il commento "Click anywhere to continue" /
